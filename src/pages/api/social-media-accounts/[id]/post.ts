    ...
    if (account.accountType === 'INSTAGRAM') {
      ...
      if (post.imageUrl) {
        try {
          ...
          logger.warn(`Could not resolve image URL for LinkedIn post: ${err.message}`, { userId: user.id });
        }
      }
      postResult = await publishLinkedInPost(account, post.caption, resolvedImageUrl, user.id);
      ...
      if (post.imageUrl) {
        try {
          ...
          logger.warn(`Could not resolve image URL for X post, continuing without media: ${err.message}`, { userId: user.id });
        }
      }
